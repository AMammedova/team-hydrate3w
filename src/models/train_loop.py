...
- CUDA AMP uses BF16 for improved numerical stability on supported hardware;
...
class Trainer:
    def __init__(
        ...
        # --------------------------------------------------------------
        #
        # Previous FP16 AMP smoke tests produced non-finite validation
        # probabilities for GRU training.
        #
        # On supported CUDA hardware such as NVIDIA A100, BF16 is preferred
        # because it retains FP32-like exponent range while still benefiting
        # from Tensor Core mixed-precision acceleration.
        #
        # CPU or --no-amp => AMP disabled.
        # CUDA + AMP      => BF16 autocast.
        # --------------------------------------------------------------
        ...
        # GradScaler is mainly required for FP16 because of its limited
        # dynamic range. BF16 normally does not require loss scaling.
        #
        # Keeping a disabled scaler allows the training code below to use
        # one consistent API for both AMP and full-precision training.
    ...
    def _run_epoch_train(
        ...
        for step, (x, mask, y) in enumerate(
            loader
        ):
            x = x.to(
                self.device,
    ...
    def _run_epoch_val(
        ...
        for x, mask, y in loader:
            ...
            # Convert logits to float32 before softmax and NumPy export.
            # This avoids propagating BF16 values into downstream metrics.
            probs = torch.softmax(
                logits.float(),
                dim=1,
            )
    ...
    def fit(
        ...
        if self._best_epoch is None:
            raise RuntimeError(
                "validation PR-AUC was undefined for every epoch, "
                "so no best checkpoint could be selected. "
                "Check the validation fold: it must contain both "
                "Normal and positive "
                "(Transient/Established) windows."
            )
    ...
    def load_last_checkpoint(
        ...
        if not path.exists():
            raise FileNotFoundError(
                f"no checkpoint to resume from at {path}"
            )

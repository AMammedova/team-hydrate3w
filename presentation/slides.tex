...
\begin{frame}{Lead Time vs. False Alarm Rate Trade-off}
  \begin{center}
    \includegraphics[width=0.82\textwidth,height=0.62\textheight,keepaspectratio]{lead_time_vs_far.png}
  \end{center}
...
\begin{frame}{Conclusion \& Recommendations}
  \begin{block}{Summary}
    \begin{itemize}
      \item XGBoost (real\_only): best recall \textbf{23.3\%}, lead time \textbf{43.4\,min}, FAR \textbf{7.9\%/h} (within budget for 2/3 folds)
  ...
  \begin{columns}
    \column{0.5\textwidth}
      \begin{exampleblock}{Recommendation for operators}
        Deploy XGBoost (real\_only) as the current best early-warning system. Expect to miss $\approx$75\% of events --- this reflects the fundamental scarcity of real labeled data, not a fixable algorithm problem.
      \end{exampleblock}
    \column{0.48\textwidth}

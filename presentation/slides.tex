...
\begin{frame}{XGBoost Baseline — Validation PR-AUC}
    ...
    \column{0.52\textwidth}
      \begin{table}
        ...
        \begin{tabular}{cccc}
          ...
          2 & 5 & 0.0750 & 0.1150 \\
          \midrule
          \multicolumn{2}{c}{mean $\pm$ sd} & $0.28 \pm 0.43$ & $0.11 \pm 0.11$ \\
          \bottomrule
        \end{tabular}
      \end{table}
    \column{0.46\textwidth}
  ...
  \end{columns}
  \vspace{0.4em}
  \textbf{Simulated data did not help XGBoost.} Paired difference real+sim $-$ real\_only = $-0.18 \pm 0.34$ (negative in 2 of 3 folds).
\end{frame}
...
\begin{frame}{Head-to-Head: Event Recall}
  \begin{columns}
    \column{0.5\textwidth}
      \begin{table}
        \caption{Event Recall at the validation-selected operating point (target $1\%$/h; realised $7.9\%$/h)}
        \begin{tabular}{lcc}
          \toprule

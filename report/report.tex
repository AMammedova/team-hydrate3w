...
XGBoost trained exclusively on real data achieved the highest event recall of $23.3\%$ ($\pm 25.2\%$) and a mean lead time of $43.4$\,min across the two folds where any event was detected. The threshold was selected on validation to meet a budget of one alarm per 100 operating hours, but that budget did not transfer: the realised test false-alarm rate was $7.9\%$/h, and no fold met the target.
...
\midrule
0 & 1 & 0.0004 & 0.0002 \\
1 & 3 & \textbf{0.7762} & 0.2111 \\
2 & 5 & 0.0750 & 0.1150 \\
\midrule
\multicolumn{2}{c}{mean $\pm$ sd} & $0.284 \pm 0.428$ & $0.109 \pm 0.106$ \\
\bottomrule
\end{tabular}
\end{table}

Simulated data did not help. The paired difference
(\texttt{real\_plus\_sim} $-$ \texttt{real\_only}) is $-0.175 \pm 0.338$,
...
scaling fit on validation reduced expected calibration error from $0.0410$
to $0.0083$ under \texttt{real\_only} and from $0.0554$ to $0.0106$ under
\texttt{real\_plus\_sim} (an 80--81\% reduction), and the Brier score by
29--30\%, improving in all 6 fold~$\times$~condition cells. Because Module~8
